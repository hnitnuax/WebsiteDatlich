...
const updateProfile = async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Hồ sơ đã cập nhật' })
    ...
    try {
        ...
        if (slots_booked[slotDate]) {
            if (slots_booked[slotDate].includes(slotTime)) {
                return res.json({ success: false, message: 'Không có sẵn lịch hẹn' })
            }
            else {
...
const cancelAppointment = async (req, res) => {
    try {
        ...
        if (appointmentData.userId !== userId) {
            return res.json({ success: false, message: 'Hành động không được cho phép' })
        }
